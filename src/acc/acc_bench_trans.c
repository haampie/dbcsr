...
int main(int argc, char* argv[])
{
...
#endif
  CHECK(c_dbcsr_acc_host_mem_allocate((void**)&mat_hst, sizeof(ELEM_TYPE) * mn * stack_size, stream), &result);
  CHECK(c_dbcsr_acc_host_mem_allocate((void**)&stack_hst, sizeof(int) * stack_size, stream), &result);
  ...
  CHECK(c_dbcsr_acc_dev_mem_allocate((void**)&mat_dev, sizeof(ELEM_TYPE) * mn * stack_size), &result);
  CHECK(c_dbcsr_acc_dev_mem_allocate((void**)&stack_dev, sizeof(int) * stack_size), &result);
#if defined(USE_LIBXSMM)
  CHECK(c_dbcsr_acc_stream_sync(stream), &result);
  start = libxsmm_timer_tick();
#endif
  CHECK(c_dbcsr_acc_memcpy_h2d(mat_hst, mat_dev, sizeof(ELEM_TYPE) * mn * stack_size, stream), &result);
  CHECK(c_dbcsr_acc_memcpy_h2d(stack_hst, stack_dev, sizeof(int) * stack_size, stream), &result);
#if defined(USE_LIBXSMM)
  CHECK(c_dbcsr_acc_stream_sync(stream), &result);
